...
public class Tire extends CarPart {
	...
	public void replacePart() {
		...
		// 32 is the maximum psi of each tire.
	}

	// Override status method to include tire number, not just serial number
	public void status() {
		System.out.println("Tire #" + this.tireNumber + " (serial #111000222" + this.serialNumber + ") is at " + this.condition
				+ this.conditionMeasure + " and " + this.inflationPSI + "psi.");
	}

	public void status(String extraMessage) {
		System.out.print("Tire #" + this.tireNumber + " (" + this.condition + this.conditionMeasure + ", "
				+ this.inflationPSI + "psi) say: ");
	...
	public void function(float milesDriven) throws CarCrashException {
		...
		this.changeCondition(-1 * (milesDriven / 1200) * rand.nextFloat());
		float pressureLoss;
		...
		if (this.condition < 12 || this.inflationPSI < 15) {
			this.crashCar();
